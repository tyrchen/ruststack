...
impl RustStackSqs {
    ...
    pub async fn send_message_batch(
        ...
        for entry in input.entries {
            ...
            match handle.send_message(send_input).await {
                Ok(output) => {
                    successful.push(ruststack_sqs_model::types::SendMessageBatchResultEntry {
                        id: entry.id,
    ...
    pub async fn delete_message_batch(
        ...
        for entry in input.entries {
            match handle.delete_message(entry.receipt_handle).await {
                Ok(()) => {
                    successful.push(ruststack_sqs_model::types::DeleteMessageBatchResultEntry {
                        id: entry.id,
                    });
                }
                Err(err) => {
    ...
    pub async fn change_message_visibility_batch(
        ...
        for entry in input.entries {
                ...
                .await
            {
                Ok(()) => {
                    successful.push(
                        ruststack_sqs_model::types::ChangeMessageVisibilityBatchResultEntry {
                            id: entry.id,
                        },
                    );
                }
        ...
        for entry in &self.queues {
            ...
            if let Ok(attrs) = attrs {
                if let Some(policy_json) = attrs.get("RedrivePolicy") {
                    if let Ok(policy) = serde_json::from_str::<
                        ruststack_sqs_model::types::RedrivePolicy,
                    >(policy_json)
                    {
    ...
    pub async fn start_message_move_task(
        ...
        Err(SqsError::new(
            ruststack_sqs_model::error::SqsErrorCode::UnsupportedOperation,
