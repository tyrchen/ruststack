...
impl std::fmt::Debug for KinesisHttpConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("KinesisHttpConfig")
...
impl Default for KinesisHttpConfig {
    fn default() -> Self {
        Self {
...
///
/// Wraps a [`KinesisHandler`] implementation and routes incoming HTTP
/// requests to the appropriate Kinesis operation handler.
#[derive(Debug)]
pub struct KinesisHttpService<H: KinesisHandler> {
    handler: Arc<H>,
    config: Arc<KinesisHttpConfig>,
}

impl<H: KinesisHandler> KinesisHttpService<H> {
    /// Create a new `KinesisHttpService`.
    pub fn new(handler: Arc<H>, config: KinesisHttpConfig) -> Self {
        Self {
            handler,
...
impl<H: KinesisHandler> Clone for KinesisHttpService<H> {
    fn clone(&self) -> Self {
        Self {
...
impl<H: KinesisHandler> hyper::service::Service<http::Request<Incoming>> for KinesisHttpService<H> {
    type Response = http::Response<KinesisResponseBody>;
...
/// Process a single Kinesis HTTP request through the full pipeline.
async fn process_request<H: KinesisHandler>(
    ...
    // 1. Verify POST method (Kinesis only accepts POST).
    if parts.method != http::Method::POST {
        let err = KinesisError::with_message(
            ruststack_kinesis_model::error::KinesisErrorCode::InvalidAction,
            format!("Kinesis requires POST method, got {}", parts.method),
        );
    ...
    if !config.skip_signature_validation {
        if let Some(ref cred_provider) = config.credential_provider {
            ...
            {
                let err = KinesisError::with_message(
                    ruststack_kinesis_model::error::KinesisErrorCode::InternalFailureException,
                    auth_err.to_string(),
                );
                return error_to_response(&err, request_id);
            }
...
async fn collect_body(incoming: Incoming) -> Result<Bytes, KinesisError> {
    incoming
        .collect()
        .await
        .map(http_body_util::Collected::to_bytes)
        .map_err(|e| KinesisError::internal_error(format!("Failed to read request body: {e}")))
}

/// Add common response headers to every Kinesis response.
fn add_common_headers(
    mut response: http::Response<KinesisResponseBody>,
    request_id: &str,
) -> http::Response<KinesisResponseBody> {
