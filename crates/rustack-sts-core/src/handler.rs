...
use bytes::Bytes;
use rustack_sts_http::{
    body::StsResponseBody,
    dispatch::StsHandler,
    request::parse_form_params,
    response::{XmlWriter, xml_response},
};
use rustack_sts_model::{error::StsError, operations::StsOperation, types::Credentials};

use crate::provider::RustackSts;

/// Handler that bridges the HTTP layer to the STS provider.
#[derive(Debug)]
pub struct RustackStsHandler {
    provider: Arc<RustackSts>,
}

impl RustackStsHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustackSts>) -> Self {
        Self { provider }
    }
}

impl StsHandler for RustackStsHandler {
    fn handle_operation(
        &self,
        op: StsOperation,
        body: Bytes,
