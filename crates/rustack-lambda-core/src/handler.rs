...
#[derive(Debug)]
pub struct RustackLambdaHandler {
    provider: Arc<RustackLambda>,
}

impl RustackLambdaHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustackLambda>) -> Self {
        Self { provider }
    }
}

impl LambdaHandler for RustackLambdaHandler {
    fn handle_operation(
        &self,
        op: LambdaOperation,
