...
impl RustackCloudWatch {
    /// Create a new `RustackCloudWatch` provider with the given configuration.
    ///
    ...
    pub fn get_metric_stream(
        ...
        let include_filters = record
            ...
            .into_iter()
            .map(
                |(ns, metric_names)| rustack_cloudwatch_model::types::MetricStreamFilter {
                    namespace: Some(ns),
                    metric_names,
                },
            )
...
mod tests {
    ...
    fn make_provider() -> RustackCloudWatch {
        RustackCloudWatch::new(Arc::new(CloudWatchConfig::default()))
    }
