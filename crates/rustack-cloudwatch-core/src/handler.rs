...
use crate::provider::RustackCloudWatch;

/// Handler that bridges the HTTP layer to the CloudWatch provider.
#[derive(Debug)]
pub struct RustackCloudWatchHandler {
    provider: Arc<RustackCloudWatch>,
}

impl RustackCloudWatchHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustackCloudWatch>) -> Self {
        Self { provider }
    }
}

impl CloudWatchHandler for RustackCloudWatchHandler {
    fn handle_operation(
        &self,
...
fn write_partial_failures_xml(
    w: &mut XmlWriter,
    failures: &[rustack_cloudwatch_model::types::PartialFailure],
) {
...
fn parse_metric_stat(params: &[(String, String)]) -> Result<Option<MetricStat>, CloudWatchError> {
    ...
    match (stat, period) {
        (Some(stat_val), Some(period_val)) => {
            ...
            Ok(Some(MetricStat {
                metric: rustack_cloudwatch_model::types::Metric {
                    namespace,
                    metric_name,
                    dimensions,
                },
...
fn parse_anomaly_detector_configuration(
    params: &[(String, String)],
) -> Result<Option<rustack_cloudwatch_model::types::AnomalyDetectorConfiguration>, CloudWatchError>
{
    let metric_timezone =
    ...
    for sub_params in &range_structs {
        ...
        excluded_time_ranges.push(rustack_cloudwatch_model::types::Range {
            start_time,
            end_time,
        });
    }
    ...
    Ok(Some(
        rustack_cloudwatch_model::types::AnomalyDetectorConfiguration {
            metric_timezone,
