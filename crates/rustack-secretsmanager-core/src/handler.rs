...
use crate::provider::RustackSecretsManager;

/// Handler that bridges the HTTP layer to the Secrets Manager provider.
#[derive(Debug)]
pub struct RustackSecretsManagerHandler {
    provider: Arc<RustackSecretsManager>,
}

impl RustackSecretsManagerHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustackSecretsManager>) -> Self {
        Self { provider }
    }
}

impl SecretsManagerHandler for RustackSecretsManagerHandler {
    fn handle_operation(
        &self,
...
fn dispatch(
    provider: &RustackSecretsManager,
    op: SecretsManagerOperation,
    body: &[u8],
    ...
    serde_json::from_slice(body).map_err(|e| {
        ...
        if msg.contains("missing field") || msg.contains("unknown variant") {
            SecretsManagerError::with_message(
                rustack_secretsmanager_model::error::SecretsManagerErrorCode::InvalidParameterException,
