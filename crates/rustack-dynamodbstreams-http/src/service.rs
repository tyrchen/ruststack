...
#[derive(Clone)]
pub struct DynamoDBStreamsHttpConfig {
...
impl std::fmt::Debug for DynamoDBStreamsHttpConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DynamoDBStreamsHttpConfig")
...
impl Default for DynamoDBStreamsHttpConfig {
    fn default() -> Self {
        Self {
...
/// Hyper `Service` implementation for DynamoDB Streams.
///
/// Wraps a [`DynamoDBStreamsHandler`] implementation and routes incoming HTTP
/// requests to the appropriate DynamoDB Streams operation handler.
#[derive(Debug)]
pub struct DynamoDBStreamsHttpService<H: DynamoDBStreamsHandler> {
    handler: Arc<H>,
    config: Arc<DynamoDBStreamsHttpConfig>,
}

impl<H: DynamoDBStreamsHandler> DynamoDBStreamsHttpService<H> {
    /// Create a new `DynamoDBStreamsHttpService`.
    pub fn new(handler: Arc<H>, config: DynamoDBStreamsHttpConfig) -> Self {
        Self {
            handler,
...
impl<H: DynamoDBStreamsHandler> Clone for DynamoDBStreamsHttpService<H> {
    fn clone(&self) -> Self {
        Self {
...
impl<H: DynamoDBStreamsHandler> hyper::service::Service<http::Request<Incoming>>
    for DynamoDBStreamsHttpService<H>
{
    type Response = http::Response<DynamoDBStreamsResponseBody>;
...
/// Process a single DynamoDB Streams HTTP request through the full pipeline.
async fn process_request<H: DynamoDBStreamsHandler>(
    req: http::Request<Incoming>,
    handler: &H,
    config: &DynamoDBStreamsHttpConfig,
    request_id: &str,
    ...
    if parts.method != http::Method::POST {
        let err = DynamoDBStreamsError::validation(format!(
            "DynamoDB Streams requires POST method, got {}",
            parts.method,
        ));
    ...
    if !config.skip_signature_validation {
        if let Some(ref cred_provider) = config.credential_provider {
            ...
            {
                let err = DynamoDBStreamsError::internal_error(auth_err.to_string());
                return error_to_response(&err, request_id);
            }
        }
    }
...
async fn collect_body(incoming: Incoming) -> Result<Bytes, DynamoDBStreamsError> {
    incoming
        .collect()
        .await
        .map(http_body_util::Collected::to_bytes)
        .map_err(|e| {
            DynamoDBStreamsError::internal_error(format!("Failed to read request body: {e}"))
        })
}

/// Add common response headers to every DynamoDB Streams response.
fn add_common_headers(
    mut response: http::Response<DynamoDBStreamsResponseBody>,
    request_id: &str,
) -> http::Response<DynamoDBStreamsResponseBody> {
