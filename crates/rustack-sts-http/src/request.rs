...
#[must_use]
pub fn parse_session_tags(params: &[(String, String)]) -> Vec<rustack_sts_model::types::Tag> {
    ...
    loop {
        ...
        match (key, value) {
            (Some(k), Some(v)) => {
                tags.push(rustack_sts_model::types::Tag { key: k, value: v });
                index += 1;
            }
            (Some(k), None) => {
                tags.push(rustack_sts_model::types::Tag {
                    key: k,
                    value: String::new(),
                });
                index += 1;
