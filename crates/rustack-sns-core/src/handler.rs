...
use crate::provider::RustackSns;

/// Handler that bridges the HTTP layer to the SNS provider.
#[derive(Debug)]
pub struct RustackSnsHandler {
    provider: Arc<RustackSns>,
}

impl RustackSnsHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustackSns>) -> Self {
        Self { provider }
    }
}

impl SnsHandler for RustackSnsHandler {
    fn handle_operation(
        &self,
        op: SnsOperation,
        body: Bytes,
    ...
    match op {
        ...
        SnsOperation::ListPhoneNumbersOptedOut => {
            ...
            for number in &output.phone_numbers {
                w.raw("<member>");
                w.raw(&rustack_sns_http::response::xml_escape(number));
                w.raw("</member>");
            }
            w.raw("</phoneNumbers>");
            w.write_optional_element("NextToken", output.next_token.as_deref());
