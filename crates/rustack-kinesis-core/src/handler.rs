...
use crate::provider::RustackKinesis;

/// Handler that bridges the HTTP layer to the Kinesis provider.
#[derive(Debug)]
pub struct RustackKinesisHandler {
    provider: Arc<RustackKinesis>,
}

impl RustackKinesisHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustackKinesis>) -> Self {
        Self { provider }
    }
}

impl KinesisHandler for RustackKinesisHandler {
    fn handle_operation(
        &self,
        op: KinesisOperation,
        body: Bytes,
