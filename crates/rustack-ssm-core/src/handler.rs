...
use bytes::Bytes;
use rustack_ssm_http::{body::SsmResponseBody, dispatch::SsmHandler, response::json_response};
use rustack_ssm_model::{error::SsmError, operations::SsmOperation};

use crate::provider::RustackSsm;

/// Handler that bridges the HTTP layer to the SSM provider.
#[derive(Debug)]
pub struct RustackSsmHandler {
    provider: Arc<RustackSsm>,
}

impl RustackSsmHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustackSsm>) -> Self {
        Self { provider }
    }
}

impl SsmHandler for RustackSsmHandler {
    fn handle_operation(
        &self,
        op: SsmOperation,
        body: Bytes,
