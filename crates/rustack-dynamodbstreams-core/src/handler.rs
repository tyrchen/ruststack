...
use crate::provider::RustackDynamoDBStreams;

/// Handler that bridges the HTTP layer to the DynamoDB Streams provider.
#[derive(Debug)]
pub struct RustackDynamoDBStreamsHandler {
    provider: Arc<RustackDynamoDBStreams>,
}

impl RustackDynamoDBStreamsHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustackDynamoDBStreams>) -> Self {
        Self { provider }
    }
}

impl DynamoDBStreamsHandler for RustackDynamoDBStreamsHandler {
    fn handle_operation(
        &self,
...
fn dispatch(
    provider: &RustackDynamoDBStreams,
    op: DynamoDBStreamsOperation,
    body: &[u8],
