//! SSM response serialization and error formatting.

use rustack_ssm_model::error::SsmError;

use crate::body::SsmResponseBody;

/// Content type for SSM JSON responses (awsJson1_1).
pub const CONTENT_TYPE: &str = "application/x-amz-json-1.1";

/// Serialize an SSM error into a JSON response body.
///
/// The error format follows the AWS SSM JSON protocol:
///
/// ```json
/// {
...
/// ```
///
/// Note: SSM uses short error type names (not fully-qualified like DynamoDB)
/// and lowercase `"message"` field.
#[must_use]
pub fn error_to_json(error: &SsmError) -> Vec<u8> {
...
/// Convert an `SsmError` into a complete HTTP error response.
#[must_use]
pub fn error_to_response(error: &SsmError, request_id: &str) -> http::Response<SsmResponseBody> {
    let json = error_to_json(error);
    let crc = crc32fast::hash(&json);
    let body = SsmResponseBody::from_json(json);
...
#[must_use]
pub fn json_response(json: Vec<u8>, request_id: &str) -> http::Response<SsmResponseBody> {
    let crc = crc32fast::hash(&json);
    let body = SsmResponseBody::from_json(json);
...
mod tests {
    ...
    fn test_should_format_error_json_with_short_type() {
        let err = SsmError::with_message(
            SsmErrorCode::ParameterNotFound,
            "Parameter /my/param not found",
        );
        let json = error_to_json(&err);
        let parsed: serde_json::Value = serde_json::from_slice(&json).expect("valid JSON");
        assert_eq!(parsed["__type"], "ParameterNotFound");
        assert_eq!(parsed["message"], "Parameter /my/param not found");
    }

    #[test]
    fn test_should_build_error_response_with_correct_status() {
        let err = SsmError::with_message(SsmErrorCode::ParameterNotFound, "not found");
    ...
    fn test_should_build_json_success_response() {
        let json = serde_json::to_vec(&serde_json::json!({"Version": 1, "Tier": "Standard"}))
            .expect("valid JSON");
