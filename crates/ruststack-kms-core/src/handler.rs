...
#[derive(Debug)]
pub struct RustStackKmsHandler {
    provider: Arc<RustStackKms>,
}

impl RustStackKmsHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustStackKms>) -> Self {
        Self { provider }
    }
}

impl KmsHandler for RustStackKmsHandler {
    fn handle_operation(
        &self,
        op: KmsOperation,
        body: Bytes,
