...
use crate::provider::RustackIam;

/// Handler that bridges the HTTP layer to the IAM provider.
#[derive(Debug)]
pub struct RustackIamHandler {
    provider: Arc<RustackIam>,
}

impl RustackIamHandler {
    /// Create a new handler wrapping a provider.
    #[must_use]
    pub fn new(provider: Arc<RustackIam>) -> Self {
        Self { provider }
    }
}

impl IamHandler for RustackIamHandler {
    fn handle_operation(
        &self,
        op: IamOperation,
        body: Bytes,
